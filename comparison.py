...
def compare(setting:str, istrain:bool=False):
    ...
    ### Load data
    X, y = get_data(csv_path=data['data'], split_ratio=data['split_ratio'], disp=False)
    ...
    for i, name in enumerate(data['optimizers'].keys()):
        ...
        acc_test, model, _LOSS, _ACCTRA, _ACCVAL = experiment(data=data['data'],
                                                                n_test=data['n_test'],
        ...
        if data['disp']:
            ...
            plt.tight_layout()
    plt.savefig('img/comparison_results.png')
    plt.show()
